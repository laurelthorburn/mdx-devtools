        ...
        {component.plugins.remark.length > 0 ? (
          <div>
            {component.plugins.remark.map((plugin) => (
              <span
                key={plugin}
                style={{ ...styles.pluginTag, ...styles.remarkTag }}
              >
                {plugin}
              </span>
            ))}
        ...
        {component.plugins.rehype.length > 0 ? (
          <div>
            {component.plugins.rehype.map((plugin) => (
              <span
                key={plugin}
                style={{ ...styles.pluginTag, ...styles.rehypeTag }}
              >
                {plugin}
              </span>
            ))}
  ...
  const toggleInspect = useCallback(() => {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      if (tabs[0]?.id) {
        chrome.tabs.sendMessage(tabs[0].id, {
          type: newState
            ? "MDX_DEVTOOLS_INSPECT_START"
            : "MDX_DEVTOOLS_INSPECT_STOP",
        });
      }
    });
  }, [isInspecting]);
    ...
    <div style={styles.container}>
      <div style={styles.toolbar}>
        ...
        <span style={{ marginLeft: "auto", opacity: 0.6 }}>
          {components.length} MDX component{components.length !== 1 ? "s" : ""}{" "}
          found
        </span>
      </div>
