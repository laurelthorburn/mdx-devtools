...
export function createOverlay(): void {
  ...
  function findById(list: MDXComponentInfo[], id: string): MDXComponentInfo | null {
    for (const c of list) {
      if (c.id === id) return c;
  ...
  function renderItem(comp: MDXComponentInfo, depth: number): string {
    ...
    return [
      '<div class="mdx-dt-item' + selected + '" data-id="' + comp.id + '" style="padding-left: ' + paddingLeft + 'px">',
  ...
  function renderDetails() {
    ...
    details.innerHTML = [
      ...
      '  <div style="color:#e0e0e0;word-break:break-all">' + comp.sourceFile + '</div>',
      '</div>',
      '<div class="mdx-dt-section">',
  ...
  function showHighlight(comp: MDXComponentInfo | null) {
    ...
    const label = highlight.querySelector('.mdx-dt-highlight-label') as HTMLElement;
